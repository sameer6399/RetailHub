...
export default async function insertOrderFromJson(requestObject) {
  try {
    ...
    const response = await fetch(config.URL + "insertOrderFromJson", requestOptions);

    if (!response.ok) {
      throw response;
    }
  } catch (err) {
    console.log("Error while calling addNewEmployee service!", err);
  }
}
